...
Everything here runs on **Node 24+** - the suite and benchmarks lean on Node's
native TypeScript type-stripping, so the `.ts` sources import directly with no
build step.
...
**131 tests** across 15 files, **130 passing + 1 skipped**. The skipped one is the
integration suite, which only wakes up when you point it at a real database (see
below).
...
| `breaker.test.ts` | the circuit breaker: open after repeated reconnect failures, half-open probe, close on success |
| `cache.test.ts` | the TTL + LRU result cache, plus pattern clear |
| `crud.test.ts` | the safe SQL builders behind the CRUD helpers - values bound, identifiers escaped |
| `replicas.test.ts` | round-robin read routing, health cooldown, and failover to the primary |
| `schema.test.ts` | schema introspection: table/column existence and row shaping |
...
| `profiler.test.ts` | latency percentiles, query-shape aggregation, per-resource breakdown, the in-flight gauge |
...
| `integration/db.test.ts` | all of the above against a **real** MySQL/MariaDB (skips without a DSN) |

### Integration tests (opt-in)

```bash
# any one (or more) of these turns the integration suite on
...
[`integration/README.md`](integration/README.md) explains what they cover, and
why the `Database` singleton's lifecycle stays unit-tested instead.

## Benchmarks

The benchmark scripts live in [`../benchmarks`](../benchmarks). The numbers below
come from a Node 24 run on Windows - **your hardware will differ**, so reproduce
them with the commands shown rather than trusting the digits.
...
memoised plan) - a rounding error next to a database round-trip (hundreds of
...
expansion, so there's no comparable parse-layer cost to race against. Either way
these are all millions of ops/s - the point isn't the raw speed (a network
round-trip dwarfs it) but that vSQL's richer parser is no longer the price.
...
pool settings. Point another resource at the same DB/table to compare
like-for-like. See [`../benchmarks/README.md`](../benchmarks/README.md) for the
full set - caching, RETURNING, batching, replicas, and pool saturation.
